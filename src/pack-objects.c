...
static int retrieve_object(git_walk_object **out, git_packbuilder *pb, const git_oid *id)
{
	...
	pos = kh_get(oid, pb->walk_objects, id);
	if (pos != kh_end(pb->walk_objects)) {
		obj = kh_value(pb->walk_objects, pos);
	} else {
		if ((error = lookup_walk_object(&obj, pb, id)) < 0)
			return error;
		kh_value(pb->walk_objects, pos) = obj;
	}
...
int git_packbuilder_fromwalk(git_packbuilder **out, git_revwalk *walk)
{
	int error;
	git_oid id;
	git_packbuilder *pb;
	...
	if ((error = git_packbuilder_new(&pb, walk->repo)) < 0)
		return error;
